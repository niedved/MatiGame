...
@property (nonatomic, strong) TMXLayer *grass;
...
//    [[SKTAudio sharedInstance] playBackgroundMusic:@"bensound-cute.mp3"];
    
    self.backgroundColor = [SKColor colorWithRed:.2 green:.3 blue:.95 alpha:1.0];
    
    self.map = [JSTileMap mapNamed:@"level1.tmx"];
    [self addChild:self.map];
    self.walls = [self.map layerNamed:@"walls"];
    self.hazards = [self.map layerNamed:@"hazards"];
    self.grass = [self.map layerNamed:@"grass"];
    self.grass.zPosition = 100;
  ...
  for (NSUInteger i = 0; i < 8; i++) {
    ...
    if (gid != 0) {
      ...
      if (CGRectIntersectsRect(playerRect, tileRect)) {
        ...
        [self colisionWithBoxes: tileCoord];
      }
    }
  }
}
...
-(void)colisionWithBoxes: (CGPoint) tileCoord{
  self.gameOver = YES;
  [self runAction:[SKAction playSoundFileNamed:@"mmm.wav" waitForCompletion:NO]];
  
 SKSpriteNode* sprite = [self.hazards tileAtCoord:tileCoord];
  [sprite removeFromParent];
  [self.hazards removeTileAtCoord:tileCoord];
  
  
  UIViewController* cur = [[UIApplication sharedApplication].keyWindow rootViewController];
  PuzzleViewController* ttg = [[PuzzleViewController alloc] initWithNibName:@"PuzzleViewController" bundle:nil];
  
  [cur presentViewController:ttg animated:YES completion:^{
    
  }];
  
//  [self performSelector:@selector(gogo) withObject:nil afterDelay:3.0];
}
